...
async function main() {
  const [owner, tipper1, tipper2, newOwner, newAccount] =
  ...
  // await contract.connect(owner).withdrawNewAccount(newAccount.address);
  // -  old owner trying to withdraw, this would only work if we havent called the updateOwner fn

  await contract.connect(newOwner).withdrawNewAccount(newAccount.address); // new owner withdrawing
  ...
  console.log("newAccount balance", await getBalance(newAccount.address));
}
